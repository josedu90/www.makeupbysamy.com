...
// Additional info
if (muji_is_off(muji_get_theme_option('header_mobile_hide_info')) && ($muji_info=muji_get_theme_option('header_mobile_additional_info'))!='') {
	...
	<div class="content_wrap">
		<div class="columns_wrap columns_fluid">
			<div class="sc_layouts_column sc_layouts_column_align_left sc_layouts_column_icons_position_left sc_layouts_column_fluid column-1_3"><?php
				// Logo
				if (muji_is_off(muji_get_theme_option('header_mobile_hide_logo'))) {
					?><div class="sc_layouts_item"><?php
						set_query_var('muji_logo_args', array('type' => 'mobile_header'));
			...
			?><div class="sc_layouts_column sc_layouts_column_align_right sc_layouts_column_icons_position_left sc_layouts_column_fluid  column-2_3"><?php
				if (muji_exists_trx_addons()) {
					...
					if (muji_is_off(muji_get_theme_option('header_mobile_hide_login'))) {
						ob_start();
						do_action('muji_action_login', array('text_login' => false, 'text_logout' => false));
						$muji_action_output = ob_get_contents();
						ob_end_clean();
						if (!empty($muji_action_output)) {
							?><div class="sc_layouts_item sc_layouts_menu sc_layouts_menu_default"><?php
								muji_show_layout($muji_action_output);
							?></div><?php
						}
					}
					// Display cart button
					if (muji_is_off(muji_get_theme_option('header_mobile_hide_cart'))) {
						ob_start();
						do_action('muji_action_cart');
						$muji_action_output = ob_get_contents();
						ob_end_clean();
						if (!empty($muji_action_output)) {
							?><div class="sc_layouts_item"><?php
					...
					if (muji_is_off(muji_get_theme_option('header_mobile_hide_search'))) {
						ob_start();
						do_action('muji_action_search', 'fullscreen', 'header_mobile_search', false);
						$muji_action_output = ob_get_contents();
						ob_end_clean();
						if (!empty($muji_action_output)) {
							?><div class="sc_layouts_item"><?php
